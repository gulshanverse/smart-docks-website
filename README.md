...
SmartDocs is an intent-first, local-first workspace for turning human image, PDF, or supported Office document goals into measured and verified results. **Phase 13 adds deterministic-first structured extraction** above the completed Phase 1–12 image, PDF, OCR, AI, safe-action, conversion, Office, collection, and workflow orchestration engines.
...
Phase 13 advanced document extraction is the current milestone. It extends Phase 12 orchestration with versioned bounded schemas, deterministic-first extraction, optional OCR and AI planning, explicit field-level provenance, confidence and uncertainty states, duplicate/conflict-aware collection aggregation, validated JSON/CSV exports, and a first-class Extract Data workspace. It does not pretend to provide arbitrary code execution, unbounded batch queues, universal Office round-trips, formula recalculation, ZIP packaging, object-level PDF compression, cloud conversion, autonomous actions, or forensic metadata guarantees. Unsupported executor capabilities fail honestly and future expansion must remain separately verified.
