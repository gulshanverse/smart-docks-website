...
SmartDocs is an intent-first, local-first workspace for turning human image, PDF, or supported Office document goals into measured and verified results. **Phase 12 adds deterministic workflow orchestration** above the completed Phase 1–11 image, PDF, OCR, AI, safe-action, conversion, Office, and collection engines.
...
Phase 12 workflow orchestration is the current milestone. It extends the Phase 11 controlled collections with a versioned deterministic DAG contract, topological dependency validation, visual step review, explicit local-versus-AI boundary disclosure, collection `FOR EACH` planning, bounded parallel execution, cancellation and failure propagation, ephemeral artifact provenance, and a safe executor handoff to existing specialized engines. It does not pretend to provide arbitrary code execution, unbounded batch queues, universal Office round-trips, formula recalculation, ZIP packaging, object-level PDF compression, cloud conversion, autonomous actions, or forensic metadata guarantees. Unsupported executor capabilities fail honestly and future expansion must remain separately verified.
