...
SmartDocs is an intent-first, local-first workspace for turning human image, PDF, or supported Office document goals into measured and verified results. **Phase 15 adds durable local projects and persistent state** above the completed Phase 1–14 image, PDF, OCR, AI, safe-action, conversion, Office, collection, workflow orchestration, structured extraction, and automation reliability engines.
...
Phase 15 persistent project workspace and durable local state is the current milestone. It extends Phase 12 orchestration, Phase 13 extraction, and Phase 14 automation with local IndexedDB-backed projects, explicit persistence consent, immutable original documents, validated document versions, project history, workflow recovery metadata, metadata-only import/export, migration and retention state, and safe deletion. See [`docs/phase-14-automation-reliability.md`](docs/phase-14-automation-reliability.md), [`docs/phase-14-security.md`](docs/phase-14-security.md), and [`docs/phase-14-browser-verification.md`](docs/phase-14-browser-verification.md). It does not pretend to provide arbitrary code execution, unbounded batch queues, universal Office round-trips, formula recalculation, ZIP packaging, object-level PDF compression, cloud conversion, autonomous actions, or forensic metadata guarantees. Unsupported executor capabilities fail honestly and future expansion must remain separately verified.
