...
The current package version is **1.0.0-rc.1**. Dependencies are pinned to exact tested versions and the repository pins pnpm `11.21.0`. The GitHub Actions workflow runs on pull requests and pushes to `main` with frozen installation, typecheck, real ESLint linting, deterministic tests, and production build.
